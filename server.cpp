...
std::wstring Compass(int angle) {
	...
	if (angle >= 360 || angle < 0)
		return L"";
	angle /= scale;
	wchar_t buffer[37] = { 0 }; // Array size should be (360 / scale) + 1 to accommodate the null terminator
	...
	// Replace the calculation with a fixed value for this example
	int homeIndex = (int)(calculateAngle((double)tel.latitude / 10000000.0, (double)tel.longitude / 10000000.0, homeLat, homeLon) / scale); // Replace with actual index calculation
...
void RenderText(HWND hwnd) {
	...
	swprintf(centerBuffer, 512, L"%s\n%s\n|\n\n\n\n%s", convert_to_wstring(tel.flightMode).c_str(), Compass(tel.heading / 100).c_str(), bottomText.c_str());
