...
void RenderText(HWND hwnd) {
	...
	if (std::regex_search(telemetry, matches, pattern))
	{
		...
		swprintf(telbuffer, 100, L"CPU %4d °C\n↓ %4d KB/s\n↑ %4d KB/s\nRSSI: %4d\nSNR: %5d", temp, read_speed, write_speed, rssi, snr);
	}
	graphics.DrawString(telbuffer, -1, &font, { 1,51 }, &outlinePen);
	graphics.DrawString(telbuffer, -1, &font, { 0,50 }, &solidBrush);
	EndPaint(hwnd, &ps);
}
